...
<section class="grid grid-cols-1 md:grid-cols-2 h-screen">
  <div class="w-full px-4 xl:px-8 py-20 mx-auto bg-white flex flex-col justify-center gap-y-4">
    ...
    <form class="space-y-5" method="POST" use:enhance={({form, data, action, cancel})=>{
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <label class="block">
          ...
          <input bind:this={user_input.firstname} bind:value={input_value.firstname} required class="input input-bordered w-full" type="text" placeholder="Ex: Djossou" name="firstname"  />
          <span bind:this={error_input.firstname} class="text-xs text-red-500"></span>
        </label>
        <label class="block">
          <span class="block mb-1 text-sm font-medium text-gray-700">Prénom(s)</span>
          <input bind:this={user_input.lastname} bind:value={input_value.lastname} required class="input input-bordered w-full" type="text" placeholder="Ex: Jean" name="lastname"   />
          <span bind:this={error_input.lastname} class="text-xs text-red-500"></span>
        </label>
      </div>
      <label class="block">
        <span class="block mb-1 text-sm text-gray-700">Nom de votre entreprise / organisation</span>
        <input bind:this={user_input.organization} bind:value={input_value.organization} required class="input input-bordered w-full" type="text" placeholder="Ex: azohoue" name="organization"   />
        <span bind:this={error_input.organization} class="text-xs text-red-500"></span>

      </label>
      <label class="block">
        <span class="block mb-1 text-sm font-medium text-gray-700">Votre adresse email</span>
        <input bind:this={user_input.email} bind:value={input_value.email} required class="input input-bordered w-full" type="email" placeholder="Ex: jean.djossou@azohoue.com" name="email"  />
        <span bind:this={error_input.email} class="text-xs text-red-500"></span>
      </label>
      <label class="block">
        <span class="block mb-1 text-sm font-medium text-gray-700">Votre mot de passe <span class="text-xs">(au moins 8 caractères, 1 lettre majuscule, 1 lettre miniscule, 1 chiffre et un caractère spécial)</span></span>
    ...
    <div class="pt-6 text-sm font-medium text-gray-700 border-t border-gray-200">
      ...
      <a href="/employers/auth/signin" class="text-red-400 hover:text-purple-900">Connectez-vous</a>
    </div>
  </div>
  <div class="px-4 py-20 space-y-10 bg-[#F8F9FD] flex flex-col justify-center sm:px-20">
    ...
    <div class="flex space-x-3">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="flex-none w-6 h-6 mt-1 text-red-400">
        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
      </svg>
      <div>
        <h2 class="text-xl font-medium text-red-400">Simplicité d'utilisation</h2>
        <p class="mt-1 text-gray-700">Créer facilement des offres d'emploi, héberger directement sur notre site web.</p>
      </div>
    </div>
    <div class="flex space-x-3">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="flex-none w-6 h-6 mt-1 text-red-400">
      ...
      <div>
        ...
        <p class="mt-1 text-gray-700">Nous stockons uniquement le nécéssaire, les documents des personnes ayant postulé à votre offre d'emploi sont visibles par vous uniquement.</p>
      </div>
    </div>
    <div class="flex space-x-3">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="flex-none w-6 h-6 mt-1 text-red-400">
      ...
      <div>
        ...
        <p class="mt-1 text-gray-700">Toutes les réponses à votre annonces seront directement envoyé dans votre boîte mail pour vous permettre de répondre plus facilement aux candidatures.</p>
      </div>
    </div>
    <div class="flex space-x-3">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="flex-none w-6 h-6 mt-1 text-red-400">
      ...
      <div>
        ...
        <p class="mt-1 text-gray-700">Visualisé le nombre de personne ayant consulter votre publication et faites-en la promotion auprès de plus de monde.</p>
      </div>
    </div>
  </div>
</section>
