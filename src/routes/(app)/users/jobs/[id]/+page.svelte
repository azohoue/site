<script lang="ts">
	...
	let othersJobs = data.jobs.items;
	const job = data.job;

	othersJobs = othersJobs.filter((j: any) => j.id != job.id);
	// console.log(othersJobs);
		...
		<div id="job-details" class="grid grid-cols-1 sm:grid-cols-[2fr,1.5fr]">
			<div class="flex flex-col w-full py-5 md:pr-4 gap-y-5 ">
				<div class="flex flex-col gap-y-1 min-w-full">
					<h3 class="text-[32px] font-semibold">{job.name}</h3>
			...
			<div class="w-full flex flex-col gap-y-4 py-5 mb-20">
				...
				{#each othersJobs as othersJob}
					...
					>
						...
						<h3 class="font-semibold text-[20px]">{othersJob.name}</h3>
