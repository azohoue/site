...
<section class="bg-[#F8F9FD] h-full">
    <div class="px-4 py-20 mx-auto max-w-7xl h-full ">
      <a href="/" title="Kutty Home Page" class="flex items-center justify-start sm:justify-center text-red-400">
      ...
      >
        <h1 class="mb-5 text-xl text-left text-gray-800 sm:text-center font-semibold">Inscrivez-vous gratuitement</h1>
        <form method="post" class="pb-1 space-y-4" use:enhance={({form, data, action, cancel})=>{
          ...
          <div class="flex flex-col gap-4 sm:grid sm:grid-cols-2 w-full">
            <label class="block">
              ...
              <input bind:this={user_input.lastname} bind:value={user.lastname} name="lastname" class="input input-bordered w-full" type="text" placeholder="Ex: Djossou"  />
            ...
            <label class="block">
              ...
              <input bind:this={user_input.firstname} bind:value={user.firstname} name="firstname" class="input input-bordered w-full" type="text" placeholder="Ex: Jean"  />
          ...
          <label class="block">
            ...
            <input name="email" bind:this={user_input.email} bind:value={user.email} class="input input-bordered w-full" type="email" placeholder="Ex: jean.djossou@azohoue.com" inputmode="email"  />
            <span class="text-xs text-red-500" bind:this={error_input.email}></span>
          </label>
          <label class="block">
            <span class="block mb-1 font-medium text-gray-700">Votre mot de passe</span>
            <input name="password" bind:this={user_input.password} bind:value={user.password} class="input input-bordered w-full" type="password" placeholder="••••••••"  />
            <span class="text-xs text-red-500" bind:this={error_input.password}></span>
          </label>
          <div class="flex flex-col items-start justify-between sm:items-center sm:flex-row">
            <label class="flex items-center">
              ...
              <span class="block ml-2 text-xs font-medium text-gray-700 cursor-pointer">J'accepte les <a href="#" class="text-red-500 underline">conditions d'utilisations</a> et la <a href="#" class="text-red-400 underline">politique de confidentialité.</a></span>
              <span class="text-xs text-red-500"></span>
            </label>
            <input type="submit" class="w-full mt-5 btn btn-error text-white sm:w-auto sm:mt-0 normal-case" value="Ouvrir mon compte" />
          </div>
        </form>
      </div>
